    ...
    pluginFactory = function methodFactory(methodName, logLevel, loggerName) {
      ...
      return function () {
        ...
        if (args.length && typeof args[0] === 'string') {
	        if (hasMessage) {
            if(options.appendExtraArguments){
              const slots = ((args[0] || "").match(/(^|\s)(%[sdj])/g) || []).length;
              
              for(let i = slots + 1; i < args.length; i++){
                args[0] += typeof args[i] === 'object' ? ' %j' : ' %s';
              }
            }
            content = content.replace(/%m/, options.messageFormatter(args[0]))
          };
